...
import { Copy, Download, Boxes, Image as ImageIcon } from 'lucide-react'
...
export function AsciiExportDialog() {
  ...
  const omissions = svgOmissions(doc)
    ...
    <Dialog title="Export" onClose={close}>
      ...
      {tab === 'image' && (
        <>
          ...
          {/* a vector file has no resolution to choose, so the row that
              chooses one is not shown rather than shown and ignored */}
          {format !== 'svg' && (
            <>
          ...
          <p className="mb-4 flex items-center gap-1 t-caption text-(--tx3) tabular-nums">
            {format === 'svg' ? `${doc.size.width} × ${doc.size.height}, any size` : `${size.w} × ${size.h} px`}
            ...
            {format === 'svg' && omissions.length > 0 && (
              <InfoTip>
                Vector cannot carry {omissions.join('; ')}. Everything else is exported as shapes.
              </InfoTip>
            )}
          </p>
          <div className="flex gap-2">
            <button
              onClick={() =>
                void run(exportShape(doc, { kind: 'clipboard', format, scale }), async () => {
                  // SVG goes on the clipboard as its own markup, which is what
                  // pastes into a design tool or an editor. Rasterising it to
                  // put a picture there instead would throw away the thing the
                  // format was chosen for.
                  if (format === 'svg') await copySvg(doc, bitmap)
                  else await copyImage(doc, bitmap, scale)
